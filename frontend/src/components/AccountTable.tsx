        ...
        {accounts.length === 0 ? (
          <div className="py-16 text-center text-gray-400">
            <div className="flex flex-col items-center gap-2">
              ...
              <p>{tx(language, "No accounts found", "Tidak ada akun ditemukan")}</p>
              <p className="text-xs">{tx(language, "Create your first account to get started", "Buat akun pertama Anda untuk memulai")}</p>
            </div>
          </div>
        ) : (
          accounts.map((account, idx) => (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: idx * 0.02 }}
              className={`p-3 ${!account.isActive ? "opacity-60" : ""}`}
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="min-w-0">
                  <span className="font-mono text-xs text-gray-700 dark:text-white bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-md">
                    {account.code}
                  </span>
                  <p className="font-medium text-gray-800 dark:text-white text-sm mt-1 truncate">{account.name}</p>
                </div>
                <StatusBadge isActive={account.isActive} />
              </div>
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <TypeBadge type={account.type} />
                  <span className={`text-xs font-medium ${account.normalBalance === "Debit" ? "text-primary-600" : "text-emerald-600"}`}>
                    {account.normalBalance}
                  </span>
                </div>
